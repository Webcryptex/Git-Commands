...
git branch -m current-branch-name new-branch-name # переименование ветки
...
git branch -D branch-name  # если не слили ветку с master, но уверены, что хотите удалить ее, можно принудительно удалить ветку.
...
git push origin --delete <branchName> # удалить удалённую ветку
git push origin -d <branchName> # удалить удалённую ветку
...
git rebase -i # убирает строки, относящиеся к ненужным коммитам. Это самое очевидное и простое решение, но у него есть недостаток, из-за которого его нельзя использовать в публичных ветвях: он изменяет историю    коммитов. Поэтому после rebase’а может возникнуть проблема с push’ем.
...
Предположим, создали коммит и забыли добавить в него какие-нибудь файлы. Теперь нужно добавить эти файлы в уже созданный коммит. Сделать это можно следующим способом.

``` bash
# Сначала добавляем в индекс файлы, которые хотим присоединить к последнему коммиту:
git add <files name или .>
# Затем выполняем команду:
git commit --amend --no-edit
# Важно, что при использовании ключа --amend, на самом деле удаляется последний коммит и создается новый коммит. Поэтому нельзя выполнять --amend для коммитов, которые уже были отправлены на удаленный репозиторий (для которых был выполнен git push). Ключ --no-edit позволяет вам изменить коммит и оставить текущее сообщение коммита без изменений.
```
